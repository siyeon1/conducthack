...
function exportEvidencePack(title, entries) {
  ...
  a.download = "shft-evidence-pack.json";
  a.click();
...
export default function ProgrammeLedger({ entries = [], title = "" }) {
  ...
  return (
    <section className="mt-5 rounded-2xl border border-line bg-paper-light shadow-card">
      <header className="flex flex-wrap items-center gap-3 border-b border-line px-5 py-4">
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-verified-tint text-verified">⛓</span>
        <div className="flex-1">
          <h2 className="text-base font-semibold text-ink">Change programme audit trail</h2>
        ...
        <button
          ...
          className="rounded-lg border border-verified/40 bg-verified-tint px-3 py-1.5 text-sm font-semibold text-verified transition hover:brightness-95"
        >
        ...
        <table className="w-full min-w-[820px] border-collapse text-sm">
          ...
          <tbody>
            {entries.map(({ nodeId, label, entry }, i) => (
              <tr key={nodeId || i} className="border-t border-line align-top">
                <td className="max-w-[190px] px-2 py-2.5 font-medium text-ink">{label}</td>
                <td className="px-2 py-2.5">
                  <span className="badge b-verified">
                    {entry.decision}
                  </span>
                </td>
                <td className="max-w-[260px] px-2 py-2.5 text-[12px] italic text-ink-soft">
                  {entry.rationale ? `“${entry.rationale}”` : <span className="not-italic text-ink-mute">—</span>}
                </td>
                <td className="px-2 py-2.5">
                  <div className="flex flex-wrap gap-1">
                    {(entry.programs || []).map((p) => (
                      <span key={p} className="rounded bg-paper-dark px-1.5 py-0.5 font-mono text-[11px] text-ink-soft">
                        {p}
                      </span>
                    ))}
                  </div>
                </td>
                <td className="px-2 py-2.5 font-mono text-[11px] text-ink-soft" title={entry.entry_hash}>
